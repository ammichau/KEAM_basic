...
%Set Parameters
%--------------------------------------------------------------------------------
global beta crra eta mu r xi z_h tau_wf alpha_e delta_e psi phi_c gam_e ybar_h  alpha_h w_scale
 ...
 phi_c = 1;       %utility weight consumption
 eta = 1.8;        % curvature of leisure
 mu = 0.4 ;        % utility weight leisure
 r = 0.0000;        % interest rate
 alpha_e = 0.026;    %Weight on hours in returns to experience
 delta_e = 0.017;   % depreciation human capital 0.017 (20% annual)
 psi = 0.4;        % curvature on hours in building experience
 ...
 z_h = 0.3; %0.01       % homeproduction productivity
 ybar_h = 0.4; %0.2    %home production constant 0.2
 alpha_h = 0.4; %0.05     %curvature on fixed type productivity in home production
 nu = 0.4; %0.4;         % parameter on search 
 nu_h = 0.6; %0.65         % curvature on hours in home production
 ym_pi = 0.8 ;     % prob. husband income
 ftWbase = 0.8;    %Base fixed type for wages
 gam_e = 0.45;      %weight on experience in wage
 tau_wf =0.7;       %wage gap

 kapbase = 1.35; %Base kappa cost 0.2
 kaplow = -0.5; %less cost for low type -0.02
 bisectTol = 0.001;
 VFtol = 0.15;
 maxViter=100;
 maxHSiter=100;
...
%Individual types: 2 wage types, 2 fixed kappa types, 4 kappa draws when young 
 ...
 %Disutility    
    ...
    kap(2,:) = kap(2,:)*0.85; %Half the cost when middle
    kap(3,:) = kap(3,:)*0.85; %Half the cost when old
    kayscale=[1.0,0.96,0.93,0.9];
...
% ----- Generate a non-linear grid for experience 
        ...
        egrid= 2*(egrid.^1.5);
%normalize wages
     w_scale=1; % maxwage=wage(ftW(nI),egrid(nE)); w_scale=1/maxwage;

%---Check looks of comparative adv in home production-------------------------------- 
 for ie=1:nE
   compadv_w(ie,1)=wage(egrid(ie),ftW(1))/hprod(ftW(1),wage(egrid(ie),ftW(1)));
   compadv_w(ie,2)=wage(egrid(ie),ftW(nI))/hprod(ftW(1),wage(egrid(ie),ftW(nI)));
   w(ie,1)=wage(egrid(ie),ftW(1));
   w(ie,2)=wage(egrid(ie),ftW(nI));
   utilWork(ie)=utilC(0.9+w(ie,2)*.7+hprod(ftW(2),wage(egrid(ie),ftW(2)))*0.3^nu_h)+utilL(0.7);
   utilNiLF(ie)=utilC(0.9+hprod(ftW(2),wage(egrid(ie),ftW(2))));
end

plot(egrid,compadv_w(:,1),'-',egrid,compadv_w(:,2),'-.')
plot(egrid,w(:,1),'-',egrid,w(:,2),'-.')
plot(egrid,utilWork(:),'-',egrid,utilNiLF(:),'-.')
clear compadv_w w
...
% ----- Value Function Iteration
        ...
        for i=1:nI
           ...
           for ie = 1:nE
                for iy = 1:nY
                    for iz = 1:nZ
                       VE0(i,it,ie,iy,iz) = utilC(wageH(it)*BCwage(iz)*ym(iy)+wage(ftW(i),egrid(ie))*0.5+hprod(ftW(i),wage(ftW(i),egrid(ie)))*((1-0.5)^nu_h))+utilL(0.5)-kap(it,i); 
                        VU0(i,it,ie,iy,iz) = utilC(hprod(ftW(i),wage(ftW(i),egrid(ie)))*0.8^nu_h+ym(iy)*wageH(nT-1)*BCwage(iz)*lamH(iz,max(iy-1,1),1));
            ...
            for iy = 1:nY
                for iz = 1:nZ
                    for ie = 1:nE   
                        ...
                        while ith<maxHSiter
                            h=0.5*(hlow+hhigh);
                        YY = wageH(it)*BCwage(iz)*ym(iy)+wage(ftW(i),egrid(ie))*h+hprod(ftW(i),wage(ftW(i),egrid(ie)))*((1-h)^nu_h);
                        YYplot(ie,iy,iz) = YY;
                        dUCdh = du_dC(YY)*(wage(ftW(i),egrid(ie))-hprod(ftW(i),wage(ftW(i),egrid(ie)))*nu_h*(1-h)^(nu_h-1));
                        dULdh = du_dL(h);
                        %Find gridpont for e' 
                        ee = exp2(egrid(ie),h); 
                        if egrid(nE)< ee
                        ...
                        %Repeat for Unemployment
                            ...
                            dVUedh(1,izz,iyy) = dVUedh(1,izz,iyy)*de2_dh(egrid(ie),h);              
                            if it+1>nT-1
                              dVUedh(2,izz,iyy)=0;  
                            else
                        ...
                        %Total derivative:
                            ...
                            for izz=1:nZ
                             ...
                             TD = TD+dUCdh+dULdh;
                        if (abs(TD)<bisectTol || ith>maxHSiter-2 ||  abs(h-hlow)<0.005)
                            gH(i,it,ie,iy,iz)=h;
                            V1 = 0;
                            for izz=1:nZ
                            for iyy=1:nY 
  ...
  for ie = 1:nE   
       %Find gridpont for e' ; independent of choices.
         ...
         %disp('experience grid is really not large enough');
           ...
           for iy = 1:nY
                for iz = 1:nZ
                    %Continuation values also independent.
                       for izz=1:nZ
                       for iyy=1:nY       
                                EVe = spline(egrid,squeeze(V(i,it,:,iyy,izz)),egrid(iee));
                                EVu = spline(egrid,squeeze(VU0(i,it,:,iyy,izz)),egrid(iee));
                                EVe2 = spline(egrid,squeeze(V(i,it+1,:,iyy,izz)),egrid(iee));
                                EVu2 = spline(egrid,squeeze(VU0(i,it+1,:,iyy,izz)),egrid(iee));
                                V1= beta*(1-piT(it))*piz(iz,izz)*EVu+beta*piT(it)*piz(iz,izz)*EVu2;
                                V2= beta*(1-piT(it))*piz(iz,izz)*EVe+beta*piT(it)*piz(iz,izz)*EVe2;
                        end
                        end
                        %Check corner
                             if V1>V2
                                h=0;
                                gS(i,it,ie,iy,iz)=h;
                                YY = wageH(it)*BCwage(iz)*ym(iy)+hprod(ftW(i),wage(ftW(i),egrid(ie)))*((1-h)^nu_h);                                                                     
                                V1 = V1+ utilC(YY)+utilL(h);
                        ...
                        while ith<maxHSiter
                            h=0.5*(hlow+hhigh);
                        YY = wageH(it)*BCwage(iz)*ym(iy)+hprod(ftW(i),wage(ftW(i),egrid(ie)))*((1-h)^nu_h);
                        dUCdh = -du_dC(YY)*(hprod(ftW(i),wage(ftW(i),egrid(ie)))*nu_h*(1-h)^(nu_h-1));
...
plot(egrid,squeeze(VU(i,1,:,1,1)),'-',egrid,squeeze(VE(i,1,:,1,1)),'-.',egrid,squeeze(V(i,1,:,1,1)),'--')  
 end
   end
...
plot(egrid(6:nE),squeeze(gS(16,2,6:nE,1,1)),'-',egrid(6:nE),squeeze(gS(16,2,6:nE,2,1)),'-.',egrid(6:nE),squeeze(gS(16,2,6:nE,1,2)),'--',egrid(6:nE),squeeze(gS(16,2,6:nE,2,2)),'-..')
...
plot(egrid,squeeze(VU(16,1,:,1,1)),'-',egrid,squeeze(VE(16,1,:,1,1)),'-.',egrid,squeeze(V(16,1,:,1,1)),'--')