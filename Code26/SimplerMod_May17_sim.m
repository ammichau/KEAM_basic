...
% cut-off for definition of NiLF
NiLFdef = 0.21; %0.3 %0.21- 1940s
NiLFbar = 0.2; % 0.2
cyclefactor = 0.5; %0.5 %0.38-1940s
...
%-------------------------------------------------------------------------------------------
%-------------------------------------------------------------------------------------------
% Compute Value Function at birth
%___________________________________________________________________________________________
%State: (nI,nT,nE,nY,nZ)
for i=1:Nsim
    %Individual type weight on low values
        iwl(i) = 2*(ftW_i(i)-0.5);
        iwh(i) = 1-iwl(i);
        e=exp_i(i,1);
        ...
        for iy=1:nY
            for iz=1:nZ
            BirthV(i,iy,iz) = iwl(i)*V(1,1,ee,iy,iz)+iwh(i)*V(2,1,ee,iy,iz);
            end
        end
end

%means
BirthV_exp =  mean(BirthV(:,1,1),'all');
BirthV_rec =  mean(BirthV(:,1,2),'all');
BirthV_hunemp =  mean(BirthV(:,3,1),'all');
BirthV_hunemprec =  mean(BirthV(:,3,2),'all');
...
NiLFwoman = zeros(Nsim*Ngen,1);
    ...
    for in=1:NgenCareer-1  %Loop over age year cohort generations. Toss ones we see too little
        ...
        elseif ageEmpR(ii,1)<cyclefactor*ageEmpR(ii,2)
            cyclewoman(ii) = 1;
        elseif EmpR_i(ii)<NiLFdef
            NiLFwoman(ii) =1;
        else
            UnDeftot(ii) = 1;
        end
...
Rec_means = {'H_Emp', 'Emp', 'UnEmp', 'NiLF', 'NonEmp', 'EE', 'EU', 'EN', 'EnonE', 'NonEE', 'Hours', 'Quit', 'Search', 'Wage', 'Household Income', 'Wife Income', 'Wife Share of Inc', 'Vfun', 'Vfun H Unemploy' ;...
            mH_Erecess , mW_Erecess, mW_Urecess, mW_Nrecess, mW_NEmprecess, m_EErecess, m_ENrecess, m_EUrecess, m_ENonErecess, m_NonEErecess, m_Hrs_recess, m_Quit_recess, m_Srch_recess, BirthV_rec, BirthV_hunemprec,...
            m_Wage_recess, m_IncHH_recess, m_IncW_recess, m_Wshare_recess  };
        
Exp_means = {'H_Emp', 'Emp', 'UnEmp', 'NiLF', 'NonEmp', 'EE', 'EU', 'EN', 'EnonE', 'NonEE', 'Hours', 'Quit', 'Search', 'Wage', 'Household Income', 'Wife Income', 'Wife Share of Inc', 'Vfun', 'Vfun H Unemploy' ;...
            mH_Eexp , mW_Eexp, mW_Uexp, mW_Nexp, mW_NEmpexp, m_EEexp, m_ENexp, m_EUexp, m_ENonEexp, m_NonEEexp, m_Hrs_exp, m_Quit_exp, m_Srch_exp, BirthV_exp, BirthV_hunemp, ...
...
j=0;
    for in=1:Ngen-5  %Loop over age year cohort generations. Toss last generation.
        ...
        for it=1:Tsim_i
            ...
            %Utility Values
                ee=find(egrid<exp_i(ii,t),1,'last');
                paneldata(j,24) = iwl(i)*V(1,Age_i(ii,t),ee,Hstat_i(ii,t),zsim(t,1))+iwh(i)*V(2,Age_i(ii,t),ee,Hstat_i(ii,t),zsim(t,1)); %value function                
                if EmpI_i(ii,t)>0
                    paneldata(j,26) =  utilL(Hours_i(ii,t));    %Disutility of hours or home production/search    
                    paneldata(j,25) = utilC(Inc_hh(ii,t)+Wage_i(ii,t)*Hours_i(ii,t)+hprod(ftW_i(i,1))*(1-Hours_i(ii,t))^nu_h);          %Value of consumption
                else
                    paneldata(j,25) = utilC(Inc_hh(ii,t)+hprod(ftW_i(i,1))*(1-Srch_i(ii,t))^nu_h);          %Value of consumption
                    paneldata(j,26) =  utilL(lhome); 
                end
            end
